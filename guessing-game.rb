...
class GuessingGame
  ...
  def pick_random_word (input_list, input_number)
    temp = []
    input_list.each do |item|
      if item.length == input_number
        temp.push item
      end
    end
  ...
  def check_word (word, letter, display_word)
    temp_list = word.downcase.split('')
    temp_value = false
  ...
  def check_letter_list (letter, list)
    list.each do |item|
      if item == letter
        return true
      end
  ...
  def run
    ...
    lives = (word_length + 5)
    puts lives.to_s
    secret_word = pick_random_word(@@AllWords, word_length)
    puts secret_word
    display_word = hide_word(secret_word)
    puts display_word.join
    puts display_word


      guessed_letter = ask_for_letters
      if check_letter_list(guessed_letter, letter_list) == false
        letter_list.push guessed_letter
        puts guessed_letter
          after_check =  check_word(secret_word, guessed_letter, display_word)
